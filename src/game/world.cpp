...
void World::init()
{
	...
	Light *l = entity_manager.make_light(Vector3(0.0f, 200.0f, 100.0f), Vector3(1.0f, 1.0f, 1.0f), Vector3(1.0f, 1.0f, 1.0f), DIRECTIONAL_LIGHT_TYPE);
	...
	//Entity *sponza = entity_manager.make_entity(ENTITY_TYPE_UNKNOWN, Vector3(0.0, 0.0, 0.0));
	//sponza->model = new Render_Model();
	////sponza->model->init_from_file("mutant.fbx");
	//sponza->model->init_from_file("erika.fbx");

	//Entity *sponza2 = entity_manager.make_entity(ENTITY_TYPE_UNKNOWN, Vector3(200.0, 0.0, 0.0));
	//sponza2->model = new Render_Model();
	//sponza2->model->init_from_file("mutant.fbx");
	//sponza->model->init_from_file("sponza.fbx");


	Entity *sponza3 = entity_manager.make_entity(ENTITY_TYPE_MUTANT, Vector3(-0.5, -1.0, 0.0));
