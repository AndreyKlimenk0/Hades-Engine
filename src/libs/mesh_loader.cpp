...
bool load_models_from_file(const char *full_path_to_model_file, Array<Loading_Model *> &models, Array<String> &textures, Loading_Models_Info *loading_models_info, Loading_Models_Options *options)
{
	...
	print("load_models: Started to load {}.", file_name);

	if (!file_exists(full_path_to_model_file)) {
		print("load_models: Failed to load. {} does not exist in model folder.", file_name);
		return false;
	}
	...
	bool result = true;
	if (!scene) {
		print("load_models: Failed to load a scene from {}.", file_name);
		result = false;
	}
