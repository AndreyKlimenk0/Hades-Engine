...
void Window::draw()
{
	...
	directx_render.draw_rounded_rect(x, y, width, height + window_draw_info.header_height, factor, factor, Color(255, 255,255));
	directx_render.draw_rounded_rect(x, y + window_draw_info.header_height, width, height, factor, factor, background_color);
	directx_render.fill_rect(x, y + window_draw_info.header_height, width, height - window_draw_info.header_height, background_color);

	
	ID2D1Bitmap *cross_bitmap = NULL;
	directx_render.load_bitmap_from_file("E:\\andrey\\dev\\hades\\data\\editor\\cross.png", 1, 1, &cross_bitmap);
	...
	directx_render.draw_bitmap(rect, cross_bitmap, scale);
...
void Button::draw()
{
	...
	directx_render.draw_rect(x, y, width, height, stroke_color);
}
