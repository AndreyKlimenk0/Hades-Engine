...
inline void Triangle_Mesh::allocate_static_buffer()
{
	...
	HR(directx11.device->CreateBuffer(&index_buffer_desc, &index_resource_data, &index_buffer));
}
...
inline void create_default_buffer(Triangle_Mesh *mesh)
{
	...
	HR(directx11.device->CreateBuffer(&vertex_buffer_desc, &vertex_resource_data, &mesh->vertex_buffer));
	...
	HR(directx11.device->CreateBuffer(&index_buffer_desc, &index_resource_data, &mesh->index_buffer));
}
#endif