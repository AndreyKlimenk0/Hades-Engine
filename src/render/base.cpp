...
void Direct3D::resize()
{
	...
	directx_render.direct2d.shutdown();
...
void DirectX_Render::draw_rounded_rect(int x, int y, int width, int height, float radius_x, float radius_y, const Color &background_color)
{
	D2D1_RECT_F rect;
	rect.left = x;
	rect.top = y;
	...
	ID2D1SolidColorBrush *brush = NULL;
	directx_render.direct2d.render_target->CreateSolidColorBrush((Color)background_color, &brush);
	...
	directx_render.direct2d.render_target->FillRoundedRectangle(rounded_rect, brush);
	END_DRAW();

	RELEASE_COM(brush);
}

void DirectX_Render::draw_bitmap(const D2D1_RECT_F &rect, ID2D1Bitmap *bitmap, float scale)
{
	BEGIN_DRAW();
	direct2d.render_target->SetTransform(D2D1::Matrix3x2F::Scale(D2D1::Size(scale, scale), D2D1::Point2F(rect.left, rect.top)));
	direct2d.render_target->DrawBitmap(bitmap, rect);
	direct2d.render_target->SetTransform(D2D1::Matrix3x2F::Identity());
	END_DRAW();
}
...
void DirectX_Render::load_bitmap_from_file(const char *file_path, int dest_width, int dest_height, ID2D1Bitmap **bitmap)
{
