...
Render_Primitive_List::Render_Primitive_List(Render_2D *render_2d, Font *font, Render_Font *render_font) : render_2d(render_2d), font(font), render_font(render_font)
{
	...
	Rect_s32 rect;
	rect.set_size(window_size.width, window_size.height);
...
void Render_Primitive_List::add_texture(Rect_s32 *rect, Texture *resource)
{
	add_texture(rect->x, rect->y, rect->width, rect->height, resource);
}

void Render_Primitive_List::add_texture(int x, int y, int width, int height, Texture *resource)
{
...
void Render_Primitive_List::add_line(const Point_s32 &first_point, const Point_s32 &second_point, const Color &color, float thickness)
{
	...
	from_win32_screen_space(window_size.width, window_size.height, first_point, converted_point1);

	Point_s32 converted_point2;
	from_win32_screen_space(window_size.width, window_size.height, second_point, converted_point2);
...
void Render_2D::init(Render_System *render_sys)
{
	u32 width = 256;
	u32 height = 256;
	...
	color_buffer.fill(Color(1.0f, 1.0f, 1.0f));
	default_texture = create_texture_from_image(&color_buffer);
...
void Render_2D::prepare_for_rendering(Render_Device *render_device)
{
	static u32 privious_total_vertex_count = 0;

	if ((total_vertex_count > 0) && (privious_total_vertex_count != total_vertex_count)) {
		...
		Primitive_2D *primitive = NULL;
		For(primitives, primitive) {
			merge(&vertices, &primitive->vertices);
			merge(&indices, &primitive->indices);
		}
		...
		vertex_buffer_desc.stride = sizeof(Vertex_P2UV);
		vertex_buffer_desc.count = total_vertex_count;
		vertex_buffer_desc.data = vertices.to_void_ptr();
		vertex_buffer_desc.name = "Render 2D vertices";

		vertex_buffer = render_device->create_buffer(&vertex_buffer_desc);

		Buffer_Desc index_buffer_desc;
		index_buffer_desc.stride = sizeof(u32);
		index_buffer_desc.count = total_index_count;
		index_buffer_desc.data = indices.to_void_ptr();
		index_buffer_desc.name = "Render 2D indices";

		index_buffer = render_device->create_buffer(&index_buffer_desc);
	}
}
...
void Render_Font::make_font_atlas(Font *font, Hash_Table<char, Rect_f32> *font_uvs)
{
	...
	font_atlas_image.create(256, 256, DXGI_FORMAT_R8G8B8A8_UNORM, "Font atlas");
	font_atlas_image.fill(Color::Black);
	...
	Rect_u32 atlas_rect;
	atlas_rect.set_size(font_atlas_image.width, font_atlas_image.height);
	...
	for (u8 c = CONTORL_CHARACTERS; c < (MAX_CHARACTERS - 1); c++) {
		...
		Rect_f32 uv;
		uv.x = static_cast<float>(rect.x) / static_cast<float>(font_atlas_image.width);
		uv.y = static_cast<float>(rect.y) / static_cast<float>(font_atlas_image.height);
		uv.width = static_cast<float>(rect.width) / static_cast<float>(font_atlas_image.width);
		uv.height = static_cast<float>(rect.height) / static_cast<float>(font_atlas_image.height);
		...
		font_atlas_image.update_region((u8 *)font_char->bitmap, rect);
	}

	Texture_Desc texture_atlas_desc;
	texture_atlas_desc.dimension = TEXTURE_DIMENSION_2D;
	texture_atlas_desc.width = 256;
	texture_atlas_desc.height = 256;
	texture_atlas_desc.miplevels = 1;
	texture_atlas_desc.format = DXGI_FORMAT_R8G8B8A8_UNORM;
	texture_atlas_desc.data = (void *)font_atlas_image.data;
	font_atlas = Engine::get_render_system()->render_device->create_texture(&texture_atlas_desc);
}
