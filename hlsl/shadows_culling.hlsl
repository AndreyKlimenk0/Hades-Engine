...
bool frustum_culled(float3 min_AABB, float3 max_AABB)
{
    ...
    for (uint i = 0; i < 8; i++) {
        float3 uv_position = clip_to_uv_coordinates(mul(float4(bounding_box_corners[i], 1.0f), pass_data.view_projection_matrix));
...
void world_aabb_to_ndc(inout float3 min_AABB, inout float3 max_AABB)
{
    ...
    for (uint i = 0; i < 8; i++) {
        float4 clip_space_position = mul(float4(bounding_box_corners[i], 1.0f), pass_data.view_projection_matrix);
...
bool occlusion_culled(float3 min_AABB, float3 max_AABB)
{
    ...
    float2 min_HZB_pixel = uv_min_AABB * pass_data.hzb_width_height_mips.xy;
    float2 max_HZB_pixel = uv_max_AABB * pass_data.hzb_width_height_mips.xy;
