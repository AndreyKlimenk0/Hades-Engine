...
Texture2D<uint> silhouette_depth_stencil_texture : register(t21);
...
void cs_main(uint3 thread_id : SV_DispatchThreadID)
{
    ...
    uint depth_sample = silhouette_depth_stencil_texture[thread_id.xy] & 0x00ffffff;    
    for (int x = -range; x <= range; x++) {
        for (int y = -range; y <= range; y++) {
            int2 offset = uint2(x, y);
            uint shifted_index_sample = silhouette_texture[thread_id.xy + offset];
            uint shifted_depth_sample = silhouette_depth_stencil_texture[thread_id.xy + offset] & 0x00ffffff;
